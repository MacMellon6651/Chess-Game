...
namespace BitboardOps {
    ...
    // Таблица для быстрого поиска индекса бита
...
namespace BitboardRows {
    ...
    static constexpr std::array<Bitboard, 8> calc_inversion_rows() {
        std::array<Bitboard, 8> rows{};
        for (uint8_t i = 0; i < 8; ++i) rows[i] = ~Rows[i];
        return rows;
    }
    static constexpr std::array<Bitboard, 8> InversionRows = calc_inversion_rows();
}

// Предвычисленные столбцы доски
